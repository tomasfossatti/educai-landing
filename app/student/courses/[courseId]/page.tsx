...
export default async function StudentCourse({params}:{params:Promise<{courseId:string}>}){
  ...
  const [conversations,sessions]=await Promise.all([
    ...
    db.classSession.findMany({where:{courseId,status:"CLOSED",interventions:{some:{insightId:{not:null}}}},include:{interventions:{where:{insightId:{not:null}},include:{insight:{include:{concept:true}}}},feedback:{where:{studentId:user.student.id},select:{conceptId:true}}},orderBy:{endedAt:"desc"}})
  ]);
  ...
  <section className="section"><div className="section-title"><div><div className="label">Después de clase</div><h2>Feedback pendiente</h2></div></div>{sessions.some(s=>s.interventions.some(i=>i.insight&&!s.feedback.some(f=>f.conceptId===i.insight!.conceptId)))?<div className="stack">{sessions.flatMap(s=>s.interventions.filter(i=>i.insight&&!s.feedback.some(f=>f.conceptId===i.insight!.conceptId)).map(i=><div className="card spread" key={`${s.id}-${i.id}`}><div><strong>{s.title}</strong><div className="small muted">Concepto: {i.insight!.concept.name}</div></div><Link className="btn secondary" href={`/student/feedback/${s.id}?concept=${i.insight!.conceptId}`}>Responder</Link></div>))}</div>:<div className="empty">No tenés feedback pendiente.</div>}</section>
