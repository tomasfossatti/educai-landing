...
export default async function StudentHome(){
  ...
  return <div className="shell student-home">
    ...
    {recentActivity&&<section className="student-primary-section" aria-labelledby="continue-title">
      ...
      <Link className="continue-card" href={`/student/chat/${recentActivity.id}`}>
        <div className="continue-card-main">
          <div className="continue-course-line"><span>{recentActivity.course.name}</span><span className="status-text">En progreso</span></div>
          <h3>{recentActivity.title}</h3>
          <p>{recentConversation?._count.messages??0} mensajes · retomá desde tu último intercambio con el tutor.</p>
        </div>
        <span className="continue-action">Continuar conversación <span aria-hidden="true">→</span></span>
      </Link>
    </section>}
    ...
    <section className="student-courses-section">
      <div className="student-courses-heading"><h2>Tus cursos</h2></div>

      <details className="join-course-disclosure course-join-action" open={!enrollments.length}>
        <summary>+ Unirme a un curso</summary>
        <div className="join-course-panel"><JoinCourseForm/></div>
      </details>

      {enrollments.length?<div className="student-course-grid">{enrollments.map(({course})=><Link key={course.id} href={`/student/courses/${course.id}`} className="student-course-card">
        <div className="student-course-card-head"><h3>{course.name}</h3><span className="course-count">{course._count.activities} actividad{course._count.activities===1?"":"es"}</span></div>
        <p>{course.description}</p>
        <span className="text-action">Abrir curso <span aria-hidden="true">→</span></span>
      </Link>)}</div>:<div className="student-empty-line">Todavía no tenés cursos.</div>}
    </section>
  </div>;
}
